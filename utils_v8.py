...
def mape_vol(y_true, y_pred):
    y_true = y_true[:,:,:,0]# * (v_stat[3] - v_stat[2]) + v_stat[2]
    y_pred = y_pred[:,:,:,0]# * (v_stat[3] - v_stat[2]) + v_stat[2]
    mape = K.mean(K.abs(y_true - y_pred) / (K.abs(y_true)+1))
    return mape

def custom_loss(y_true, y_pred):
    #return rmse_vol(y_true, y_pred) + smape_vol(y_true, y_pred)
    return rmse_vol(y_true, y_pred) + mape_vol(y_true, y_pred)
